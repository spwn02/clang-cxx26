...
void test(int a, Foo b, int *d, Foo *e, const Foo *f) {
  __builtin_clear_padding(a); // expected-error {{passing 'int' to parameter of incompatible type 'void *': type mismatch at 1st parameter ('int' vs 'void *')}}
  __builtin_clear_padding(b); // expected-error {{passing 'Foo' to parameter of incompatible type 'void *': type mismatch at 1st parameter ('Foo' vs 'void *')}}
...
void testMemberPointer(Foo* Bar::*mp) {
  __builtin_clear_padding(mp); // expected-error {{passing 'Foo *Bar::*' to parameter of incompatible type 'void *': type mismatch at 1st parameter ('Foo *Bar::*' vs 'void *')}}
}
