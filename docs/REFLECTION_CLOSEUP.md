...
| 5b | Issue #181 — non-copyable tuple/range element expansion binding | Not started | Independent of 5a (correctly a `template for` / expansion-statement binding defect). `makeCXXDestructurableExpansionSelectExpr` (`SemaExpand.cpp:245-285`) has `// TODO: Add ref support` — only turns the `auto` decomposition into a reference when the written expansion variable is itself a reference, so all of `auto`/`auto&`/`auto&&`/const forms fail for a non-copyable element (`std::tuple<std::unique_ptr<int>, char, double, float>` from the upstream repro). P1306R5's rule (from `docs/reflection-audit/codex-m4-180-181-report.md` §2): the selected element is `__v_i` when the referenced element is an lvalue reference or the initializer is an lvalue, otherwise `std::move(__v_i)`. Also check whether the hidden `__range`'s `withConst()` (`SemaExpand.cpp:150`) is load-bearing for this fix or independent before touching it. |
