...
| [~] | P3179R9 | Parallel range algorithms | Partial. **55 algorithms done as of 2026-09-16** — the entire previously-identified named-candidate pool from the 2026-09-16 scoping pass is now exhausted: `all_of`/`any_of`/`none_of`/`for_each`/`for_each_n`/`find`/`find_if`/`find_if_not`/`count`/`count_if`/`equal`/`copy`/`copy_n`/`rotate_copy`/`reverse_copy` (15, pre-existing); `fill`/`fill_n`/`generate`/`generate_n`/`is_partitioned`/`merge`/`move`/`replace`/`replace_if`/`replace_copy`/`replace_copy_if`/`sort`/`stable_sort`/`transform` (14, mechanical transplant from an existing classic overload); `adjacent_find`/`mismatch`/`search`/`find_first_of`/`find_end`/`contains`/`starts_with`/`ends_with` (8, from-scratch classic-layer composition on top of `__find_if`); `remove`/`unique`/`reverse`/`rotate`/`shift_left`/`shift_right`/`swap_ranges`/`partition` (8, sequential-fallback backends — registered under `__default_backend_tag`, not `__serial_backend_tag`, since this build's actual `__current_configuration` never reaches the latter); `set_difference`/`set_intersection`/`set_symmetric_difference`/`set_union`/`is_heap`/`is_heap_until` (6, same sequential-fallback pattern); `min_element`/`max_element`/`minmax_element`/`lexicographical_compare` (4, same pattern). `rotate_copy`/`reverse_copy` also close P3709R2, see its row. **`make_heap`/`push_heap`/`pop_heap`/`sort_heap` are confirmed out of scope entirely** — verified against cppreference that none of the four ever received a C++17 execution-policy overload (only the two heap *checks*, `is_heap`/`is_heap_until`, did) — do not attempt these. **Denominator note (corrected 2026-09-16)**: fetched the current C++26 working draft directly (per-algorithm-family subclauses at eel.is/c++draft/, since the full synopsis/overloads pages are too large to fetch whole) and found **~14 more algorithms genuinely in scope but not yet implemented**: `copy_if`, `stable_partition`, `partition_copy`, `partial_sort`, `partial_sort_copy`, `is_sorted`, `nth_element`, `inplace_merge`, `includes`, `search_n`, `find_last`, `find_last_if`, `find_last_if_not`, `contains_subrange` — each a mechanical sibling of an already-implemented family (spot-checked `copy_if`/`is_sorted` against cppreference; `is_sorted_until` was initially miscounted as in-scope and excluded after that check). **True count is ~69, not 55** — issue #13 stays open with this corrected list, not closed. Verification this pass caught 7 real bugs before merging (a `ranges::transform` two-range end-position bug, a `search` backend nullopt/not-found conflation, a `__pstl_counting_iterator` double-dereference bug affecting 4 search-family backends, backends registered under the wrong tag entirely, a reversed `indirect_unary_predicate` argument order, a wrong `set_difference` return type, and a swapped-comparator-argument bug in `max_element`) plus several test-authoring bugs (buffer/array sizes not matching their content, assertions relying on "valid but unspecified" post-mutation state) — see the Session Log below and issue #13's comment history for detail. Tracked in issue #13, which stays open pending the remaining greenfield uncertainty. |
