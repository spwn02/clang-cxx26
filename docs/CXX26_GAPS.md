...
- ~~**P2781R9 `constant_wrapper`**~~ → P3222R0 → P2642R6 / P3355R2 → P1673R13's
  ...
  here.
- **P3068R6** (throwing in constant evaluation): **Implemented and verified
  2026-09-07.** From-scratch implementation in the classic evaluator
  (`clang/lib/AST/ExprConstant.cpp`) — `EvalInfo::PendingException` tracks an
  in-flight thrown value, `VisitCXXThrowExpr` evaluates the operand and holds
  it without diagnosing yet, `EvaluateStmt`'s `CXXTryStmtClass` case matches
  it against handlers in declaration order ([except.handle]p3, static-type
  matching only), and a single, minimal change to `ScopeRAII::~ScopeRAII()`
  (run destructors instead of always skipping them when an exception is
  propagating) gets destructor-during-unwind correctness for free from the
  native C++ call stack unwinding through the evaluator's own RAII scopes.
  Full `check-clang` gate clean (only genuinely pre-existing failures,
  isolated via `git stash` + rebuild). See Tier 7 for the full design
  writeup, the two documented safe under-approximations (rethrow,
...
- **`std::simd`** — 7 papers `|In Progress|`, code in `libcxx/include/__simd/`.
  ...
  P2933R4 corrected same day — its apparent bit-ops gap was C++29 content
...
| [x] | P3179R9 | Parallel range algorithms | Complete as of 2026-09-17. **69 algorithms done** — the 14 found missing on 2026-09-16 (`copy_if`, `stable_partition`, `partition_copy`, `search_n`, `find_last`, `find_last_if`, `find_last_if_not`, `partial_sort`, `partial_sort_copy`, `is_sorted`, `nth_element`, `inplace_merge`, `includes`, `contains_subrange`) landed across two sequential Codex dispatches (wip/13-dispatchG, wip/13-dispatchH), each reviewed and corrected before merge — real bugs caught: `find_last`/`find_last_if`/`find_last_if_not`'s first draft bypassed the PSTL backend dispatch with a hand-rolled loop ignoring the execution policy (fixed by composing through the already-PSTL-backed `ranges::find`/`find_if`/`find_if_not` on a reversed range); `search_n`'s policy overload returned the wrong subrange span; `stable_partition`'s policy overload returned a plain iterator instead of the correct `subrange`; `contains_subrange`'s range-taking overload used `(*this)` inside a `static` member function. Verified via a real rebuild + full `std/algorithms/` sweep (382/382) after each merge. The prior 55 algorithms (`all_of`/`any_of`/`none_of`/`for_each`/`for_each_n`/`find`/`find_if`/`find_if_not`/`count`/`count_if`/`equal`/`copy`/`copy_n`/`rotate_copy`/`reverse_copy`/`fill`/`fill_n`/`generate`/`generate_n`/`is_partitioned`/`merge`/`move`/`replace`/`replace_if`/`replace_copy`/`replace_copy_if`/`sort`/`stable_sort`/`transform`/`adjacent_find`/`mismatch`/`search`/`find_first_of`/`find_end`/`contains`/`starts_with`/`ends_with`/`remove`/`unique`/`reverse`/`rotate`/`shift_left`/`shift_right`/`swap_ranges`/`partition`/`set_difference`/`set_intersection`/`set_symmetric_difference`/`set_union`/`is_heap`/`is_heap_until`/`min_element`/`max_element`/`minmax_element`/`lexicographical_compare`) landed in the earlier 2026-09-16 pass; see the Session Log and issue #13's full comment history for that pass's own bug list. **`make_heap`/`push_heap`/`pop_heap`/`sort_heap` remain confirmed out of scope entirely** — verified against cppreference that none of the four ever received a C++17 execution-policy overload (only the two heap *checks* did). Issue #13 is closed. |
...
| [x] | P3709R2 | Reconsider parallel `ranges::rotate_copy` / `reverse_copy` | **Complete 2026-09-16.** Both execution-policy overloads now return `rotate_copy_truncated_result`/`reverse_copy_truncated_result` (`in_in_out_result` aliases) instead of the plain 2-field result, reporting per-subrange progress. This fork's PSTL backend (`__pstl::__handle_exception`) is all-or-nothing — no partial/truncated outcome is ever actually observable — so on every (the only possible) success the fields report full completion (`in1`/`in2` at their natural endpoints); this is a correct representation given the backend's real capabilities, not an approximation. See `__algorithm/ranges_rotate_copy.h`/`ranges_reverse_copy.h`. |
...
| Status | Paper | Feature | Notes |
|---|---|---|---|
| [x] | P0528R3 | Padding bits in atomic compare-and-exchange | **Complete 2026-09-21.** Ported Clang's `__builtin_clear_padding` from upstream commit `698d44bf9ff6`, including recursive record/array/union/bit-field/vector and scalar-padding handling. `atomic<T>` now normalizes padding at construction, deprecated `atomic_init`, store, and exchange, and uses the same value-representation retry algorithm as `atomic_ref<T>` for weak and strong compare-exchange. Constant evaluation bypasses the non-constexpr builtin while preserving P3309R3 behavior. Added padded-struct coverage for construction/init/store/exchange and weak/volatile-strong CAS. Verified focused Clang builtin tests, clean libc++ rebuild, all 165 atomics tests (154 passed, 11 unsupported), and 9,826 Clang Sema/CodeGen tests with only the known `cxx2b-consteval-propagate.cpp` baseline failure. |
...
| [x] | P3222R0 | Transposed special cases for P2642 mdspan layouts | Complete 2026-09-14 — linalg::transposed preserves padded layout specialization |
...
| [x] | P3378R2 | `constexpr` exception types | **Complete 2026-09-12.** `logic_error`, `runtime_error`, and their derived generic-error types now have C++26 constexpr constructors, copies, assignments, destructors, and `what()`. Constant evaluation uses an independently allocated character copy; ordinary execution retains the existing packed refcounted-string representation. The old out-of-line definitions remain compiled in dedicated legacy-definition TUs for ABI compatibility, while new headers dispatch runtime operations through helpers. The `std::string` constructors are defined at the end of `<string>`, after `basic_string` is complete, preserving the required `<stdexcept>`/`<string>` include cycle. |
...
| [x] | P2933R4 | Extend `<bit>` with overloads for `std::simd` | **Corrected 2026-09-07 — false gap, already Complete.** The 2026-09-06 audit found `bit_reverse`/`shl`/`shr`/`bit_repeat`/`bit_compress`/`bit_expand` "missing" and blamed a missing scalar `<bit>` prerequisite. Verified via each function's actual governing paper's GitHub issue labels: all 6 belong to three separate, plenary-approved **C++29** papers (P3104R6 "Bit permutations", P3793R2 "Better shifting", P3772R2 "std::simd overloads for bit permutations") — none labeled C++26. `eel.is`'s live master draft already absorbs plenary-approved C++29 content into `[bit.shift]`/`[bit.permute]`/`[simd.bit]`, the same "draft ≠ your target standard" trap this fork's docs already warn about for P2781R9's wording. P2933R4 itself only adds `std::simd` overloads for the pre-existing C++20/23 `<bit>` functions (`byteswap`/`bit_ceil`/`bit_floor`/`has_single_bit`/`rotl`/`rotr`/`bit_width`/`countl_zero`/`countl_one`/`countr_zero`/`countr_one`/`popcount`), all already implemented in `__simd/bit.h`. Nothing to do for C++26; revisit the 6 functions only if/when this fork tracks C++29. |
| [ ] | P2664R11 | Extend `std::simd` with permutation API | **Audited 2026-09-06 — real gap: present but wrong, still open.** Static/dynamic `permute`, `compress`, `expand`, gather, scatter are all implemented (`__simd/permute.h`), but the adopted static-permute wording mandates every generated index be a constant expression in `{zero_element, uninit_element} ∪ [0, V::size())` — the implementation explicitly documents (at `permute.h:54`) that it does not enforce this for non-`constexpr` generators. A real conformance gap, not a missing-feature one — the value semantics are already correct (three-way `zero_element`/`uninit_element`/in-range return, runtime-asserted), what's missing is compile-time Mandates enforcement for a non-constexpr-callable generator. **Re-assessed 2026-09-07 with a concrete design attempt, not just deferred on sight.** The generator (`_IdxMap&& __idxmap`) arrives at `permute()` as an ordinary runtime function parameter, so the mechanism that closed P2757R3's analogous-looking problem earlier this same session (an immediate-escalating `consteval` helper, which forces a call site into being consteval-only when the sub-call can never produce a constant) does not generalize here: that trick worked because `check_dynamic_spec`'s consteval sub-call depended only on *template type arguments*, reachable without needing any runtime object to already be a compile-time constant. Verifying `__idxmap(i)`'s actual per-index *value* requires the `__idxmap` object itself to be a compile-time constant at the call site, which it structurally isn't once it's bound to an ordinary parameter — no SFINAE/requires-clause trick can observe that either, since `requires` expressions check well-formedness, not constant-evaluability. Confirmed this is not a novel obstacle to this fork: `basic_vec.h`'s own generator constructor (`[simd.ctor]/8`, `__vec_generator_ok`) has the *identical shape* of requirement and only verifies SFINAE-style callability/return-type validity, never per-index constexpr-ness — and separately documents its own `KNOWN DEVIATION` (`basic_vec.h:199`, `[simd.ctor]/12.2-3`, `ranges::size(r)` being a constant expression) for the same underlying reason, in the same header. This needs real core-language support (something in the shape of what forced-consteval or reflection-based introspection might eventually provide), not library-level cleverness — closing this out as documented-and-deliberately-deferred, not silently dropped. |
...
| [x] | P3068R6 | Allowing exception throwing in constant-evaluation | **Implemented from scratch 2026-09-07** (upstream has none of this either, confirmed via this fork's own `clang/www/cxx_status.html`). Design, entirely in the classic evaluator (`clang/lib/AST/ExprConstant.cpp`), none of it touching the reflection evaluator's stable public API: <br>• `EvalInfo::PendingException` (an `optional<{APValue Value; QualType Ty; const Expr *ThrowExpr; SmallVector<PartialDiagnosticAt,4> CallStackNotes;}>`) tracks an exception currently propagating through the evaluation. <br>• `ExprEvaluatorBase::VisitCXXThrowExpr` (the shared CRTP base, so every evaluation context is covered for free) evaluates the throw operand as an ordinary rvalue, snapshots the current call-stack backtrace via a new `CaptureCallStackNotes` helper (a caller-buffer-targeted reimplementation of the private `State::addCallStack`, needed because every `CallStackFrame` between the throw and the top-level evaluation unwinds off the native stack long before it's known whether the exception is actually uncaught — capturing lazily at diagnose-time would find nothing), and stores it in `PendingException` **without** diagnosing — whether this is ill-formed is only known once an enclosing handler has had a chance to catch it. <br>• `EvaluateStmt`'s `CXXTryStmtClass` case (previously: evaluated the try-block and unconditionally ignored every handler) now walks handlers in declaration order on `ESR_Failed`+`PendingException`, matching via a new `HandlerCanCatch` (an AST-only, `Sema`-independent reimplementation of `Sema::handlerCanCatch`'s [except.handle]p3 logic: exact type, or reference/value catch of an unambiguous public base via `CXXRecordDecl::isDerivedFrom`, or `catch(...)`), binds the caught value (by-value catches copy a possibly-base-sliced copy via a new `ExtractCaughtSubobject` helper that indexes `APValue::getStructBase`; by-reference catches instead give the exception object real storage keyed by the *original throw operand* — an `LValueBase`'s effective type is permanently tied to its key expression's own static type, re-derived from scratch on every reload, so no synthetic key will do — then apply an ordinary derived-to-base cast via the existing `CastToBaseClass` when the handler catches a base rather than the exact type), evaluates the matched handler block, and on no match leaves `PendingException` set so `ESR_Failed` keeps propagating for free exactly as it already did. <br>• **The one genuinely subtle correctness point, solved with a single minimal change**: block-scope destructors (`ScopeRAII::~ScopeRAII()`) previously always skipped running destructors on the implicit-destructor fallback path (correct for abandoning evaluation on a hard error, wrong for a real exception unwind) — changed the one line to `destroy(/*RunDestructors=*/Info.PendingException.has_value())`. Because `ScopeRAII` objects are real native C++ stack variables inside `EvaluateStmt`/`HandleFunctionCall`/etc., this one change gets correct destructor-during-unwind semantics through arbitrarily deep nested scopes and function calls for free, as the native call stack unwinds through each `return ESR_Failed;` — no need to touch the ~16 call sites that check `Scope.destroy()` individually. Verified with a destructor that mutates a shared counter, unwound through two nested scopes. <br>• `CheckUncaughtException` (structurally mirroring the existing `CheckMemoryLeaks`, but a hard `FFDiag` failure rather than a soft `CCEDiag`, since 2.24/2.25 are unconditional) is wired into all 6 places that can be an evaluation's outermost checkpoint: `EvaluateAsRValue`, `Expr::EvaluateAsConstantExpr`, `VarDecl::checkInitIsICE`'s init-path, `EvaluateCharRangeAsStringImpl`, `Expr::isPotentialConstantExpr` (the `-Winvalid-constexpr` "never produces a constant expression" checker — genuinely needed a fix too, since without it an always-throwing, always-uncaught function was silently treated as "potentially constant"), and `EvaluateDestruction` (the separate "has constant destruction" check for `[temp.param]p8` template-parameter objects). At 4 of those a naive placement is *dead code*: `Evaluate()`'s own early `if (!...) return false;` already fires first on genuine propagation, so `CheckUncaughtException` there only fires the intended diagnostic when the underlying `Evaluate()` call itself still returns success (an outstanding-but-technically-successful case, matching the leak-check's own shape) — verified this empirically (a debug build with instrumentation showed zero hits at those checkpoints for an ordinary uncaught-propagation test) and restructured each to check-then-bail so the diagnostic isn't silently lost. <br>**Two real, non-obvious bugs found and fixed by the "genuinely 0 bugs" full-gate discipline, not by design review**: (1) `EvaluateIgnoredValue` (and the identical pattern in `DataRecursiveIntBinOpEvaluator`'s comma/logical-op short-circuit handling) treats a failed sub-evaluation as "might have had a side effect we can't see, keep going" — correct for an ordinary discarded-value failure, but it silently swallowed a real in-flight exception and let execution continue past the `throw` as if nothing happened, silently producing a *wrong value* instead of failing; fixed by checking `Info.PendingException` first and propagating `false` instead. (2) The first backtrace-preservation design — diagnosing eagerly inside `HandleFunctionCall` while the call stack was still alive, so `addCallStack`'s notes wouldn't be empty — broke the cross-function-catch case outright: `Info.FFDiag`'s underlying `diag()` commits a diagnostic to `EvalStatus.Diag` immediately, and a **stale committed diagnostic silently corrupts an evaluation that later succeeds** because of the throw being genuinely caught by the caller. Root cause confirmed via debug instrumentation, not guessed. Fixed by snapshotting the backtrace into `PendingException.CallStackNotes` at throw time (via the new `CaptureCallStackNotes`) and only replaying it (`Info.addNotes`) from `CheckUncaughtException` if the exception is confirmed uncaught — diagnosis and backtrace-capture are now fully decoupled in time. <br>**Deliberately out of scope, as documented safe under-approximations** (reject a valid program rather than risk accepting an invalid one): bare `throw;` (rethrow) — would need tracking "the exception currently being handled" as a notion distinct from `PendingException`; pointer and pointer-to-member exception types — `HandlerCanCatch` treats these as "does not catch" rather than implementing the full pointer-conversion matching rules, so such an exception keeps propagating uncaught rather than risking a wrong match. **One documented, accepted diagnostic-precision loss**: `-Winvalid-constexpr`'s "never produces a constant expression" check is deliberately single-call-depth (`CheckCallLimit` refuses to execute *any* nested constexpr call while checking, by design, predating this work) — a throw whose operand needs a nested user-defined-constructor call is now invisible to that speculative check where it previously wasn't (the old code rejected every `CXXThrowExpr` unconditionally, without ever trying to evaluate its operand, so it never hit this pre-existing limitation). The real evaluator, exercised by an actual use (`static_assert`, a `constexpr` initializer, …), still correctly rejects these with a full diagnostic and backtrace — confirmed directly. The ByteCode interpreter (`-fexperimental-new-constant-interpreter`) is untouched by design (per the plan's own sequencing — prove the design once on the classic evaluator first); both interpreters are exercised by the shared `AST/ByteCode/*.cpp` test suite and correctly diverge on this feature there (split `both-note` into `expected-note`/`ref-note` in the 2 affected files). Full `check-clang` gate clean: only the 7 confirmed-pre-existing failures (isolated via `git stash` + clean rebuild before touching anything) remain. Dedicated test suite added: `clang/test/SemaCXX/constexpr-p3068r6-throw.cpp` (basic throw/catch by value and reference, uncaught propagation, `catch(...)`, base/derived matching and slicing, nested try/catch, cross-function propagation, destructor-during-unwind, handler-order, rethrow rejection, pointer-under-approximation, throw-in-loop, and a new exception thrown from within a catch handler). P3378R2 (`constexpr` exception types) remains deferred — separate library-side ABI restructure, not blocked by this. |
...
- **2026-09-14 (issue #14 phases 4–7)**: Completed P2642R6 padded layouts,
  ...
  prose audit remains for phase 8.
...
- **2026-09-07**: Finished Rank 2 and Rank 3, per explicit user instruction,
  ...
  **Deliberately deferred, not silently dropped**: P3068R6/P3378R2 —
  ...
  (**Update 2026-09-07, later session**: P3068R6 was subsequently
  implemented and verified — see its Tier 7 row for the full design.
  P3378R2 was subsequently completed 2026-09-12 with an ABI-preserving
  constexpr/runtime storage split.)
...
- **2026-09-12**: Completed P3378R2 constexpr generic exception types.
  C++26 headers use an `if consteval` message copy; runtime retains the
  packed refcounted representation and old out-of-line ABI symbols through
...
- **2026-09-16 (issue #13, P3179R9 slice)**: Added execution-policy overloads
  to ranges `find`, `find_if`, `find_if_not`, `count`, `count_if`, `equal`,
  `copy`, and `copy_n`, with focused tests. Approximately 80 other parallel
  range algorithms remain unfinished; P3179R9 remains open.
- **2026-09-16 (issue #13/#20, rotate_copy/reverse_copy slice)**: Added
  execution-policy overloads for `ranges::rotate_copy`/`ranges::reverse_copy`,
...
- **2026-09-21 (issue #81)**: Implemented P0528R3 across compiler and library.
  Ported Clang's `__builtin_clear_padding`, then normalized `atomic<T>` storage
  at construction/init/store/exchange and made both CAS forms compare value
