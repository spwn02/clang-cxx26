...
- **P3068R6** (throwing in constant evaluation): **Implemented and verified
  ...
  precision loss. → P3378R2 `constexpr` exception types remains deferred —
  a separate library-side ABI restructure, not blocked by this.
...
| [!] | P3378R2 | `constexpr` exception types | **Session-sized, deliberately deferred.** Its language-side prerequisite, P3068R6 (throwing in constant evaluation), was implemented and verified 2026-09-07 — see its row below. P3378R2 itself remains a separate library-side ABI restructure (reference-counted-string storage in `logic_error`/`runtime_error` isn't `constexpr`-compatible, `.cpp`-only `what()`/dtor implementations need compatibility symbols if moved to headers, `<stdexcept>`/`<string>` circular-include reordering), not a bug fix — revisit in a dedicated session now that its prerequisite is done. |
...
- **2026-09-07**: Finished Rank 2 and Rank 3, per explicit user instruction,
  ...
  **Deliberately deferred, not silently dropped**: P3068R6/P3378R2 —
  ...
  P3378R2 remains deferred as its own separate library-side ABI restructure.)
