...
| [~] | P3179R9 | Parallel range algorithms | Partial. **37 algorithms done as of 2026-09-16**: `all_of`/`any_of`/`none_of`/`for_each`/`for_each_n`/`find`/`find_if`/`find_if_not`/`count`/`count_if`/`equal`/`copy`/`copy_n`/`rotate_copy`/`reverse_copy` (15, earlier); `fill`/`fill_n`/`generate`/`generate_n`/`is_partitioned`/`merge`/`move`/`replace`/`replace_if`/`replace_copy`/`replace_copy_if`/`sort`/`stable_sort`/`transform` (14, mechanical transplant from an existing classic overload); `adjacent_find`/`mismatch`/`search`/`find_first_of`/`find_end`/`contains`/`starts_with`/`ends_with` (8, from-scratch — the first 5 needed a new classic PSTL backend composed on top of the existing `__find_if` primitive via a new `__pstl_counting_iterator` helper, following the `is_partitioned`/`any_of` composition pattern in `__pstl/backends/default.h`; the last 3 needed no new primitive, delegating directly to `find`/`mismatch`/`equal`'s policy overloads). `rotate_copy`/`reverse_copy` also close P3709R2, see its row. **Denominator correction (2026-09-16): the "~78 remaining" figure was never sourced and should not be repeated.** P3179R9's own wording diff is too large to fetch in full via available tooling, and no reliable secondary source gives an exact per-algorithm list, so the true total is genuinely unconfirmed. Concretely remaining: `remove`, `unique`, `reverse`, `rotate`, `shift_left`, `shift_right`, `swap_ranges`, `partition`, the 4 set operations, the 6 heap operations, `min_element`/`max_element`/`minmax_element`, `lexicographical_compare` — 20 named candidates, not a confirmed final count. Several `ranges_*.h` files in the tree are very likely out of scope entirely (no C++17 classic execution-policy precedent exists for `binary_search`/`lower_bound`/`upper_bound`/`equal_range`, `next_permutation`/`prev_permutation`, `sample`/`shuffle`, or `clamp`) and should not be counted toward the denominator without confirming against the paper's actual wording diff first. Tracked in issue #13. |
