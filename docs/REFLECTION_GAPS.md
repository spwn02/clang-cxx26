...
| CWG 3111 (array-type template parameter objects) | **Confirmed-Open, audited 2026-09-10, fix scoped not yet built/tested** | Fetched full DR text (`cplusplus.github.io/CWG/issues/3111.html`). Resolution adds [temp.param]p13 wording that array-typed template parameter objects can exist but are never denoted by an id-expression naming a constant template parameter, and changes [meta.reflection.queries]p7's (`reflect_constant`'s) Effects to special-case array types: `else if constexpr (is_array_type(type_of(R))) return reflect_constant_array([:R:]);`. Audited the fork's `reflect_constant` end-to-end: compiler-side `reflect_result` (`clang/lib/AST/ExprConstantMeta.cpp:5775-5848`) only special-cases `ReflTy->isRecordType()` (creating a `TemplateParamObjectDecl`, line 5840-5845) — array types fall through to `Arg.Lift(ReflTy)` with no backing declaration at all, unlike the record-type path. Worse, the fix is currently unreachable/broken from the library side too: `libcxx/include/meta`'s own top-of-file synopsis declares `reflect_constant(const T&) -> info` (line 91, matching the standard), but the actual definition (line 1605) is `consteval auto reflect_constant(T r)` — **by value, not `const T&`**. A by-value array parameter undergoes array-to-pointer adjustment, so `T` can never deduce to (or be usably instantiated at) an array type through this signature — the synopsis/definition mismatch is an independent, pre-existing bug that also happens to block any array-type `reflect_constant` call from ever reaching the array branch this DR requires. **Fix, two parts, needs a build+test cycle** (queued for after the M3 escalation-cluster Codex session currently in flight, to respect the plan's build-serialization rule): (1) change `reflect_constant`'s definition to `const T& r` matching its own synopsis and the standard; (2) in `reflect_result`, add an array-type branch mirroring the record-type one — delegate to the same `reflect_constant_array`-style substituted-`FixedArray` representation (or call it directly from the library wrapper once (1) makes array `T` reachable, whichever is simpler once actually attempted) rather than falling through to an unbacked `Arg.Lift`. |
| LWG 4432 (element init for `reflect_constant_array`) | **Likely-fine for the common case, one unverified proxy-reference edge, audited 2026-09-10** | Fetched full DR text (`cplusplus.github.io/LWG/issue4432`). Resolution: defines `_ei_` as `static_cast<T>(*_iti_)` (an explicit direct-init/static_cast from each range element, addressing "proxy reference ambiguity" — a range whose `reference` type isn't plain `T&`, e.g. `vector<bool>` or a `transform_view`), then simplifies `reflect_constant`'s call to take the pre-converted `_ei_` directly, plus a Throws-clause clarification. Fork's implementation (`libcxx/include/meta:2804-2814`): `for (const auto &V : elems) Args.push_back(reflect_constant(V));` — binds `V` to each element by reference with **no explicit `static_cast<ValTy>` conversion**, then lets `reflect_constant`'s own template deduction infer `T` from `V`'s actual type. For an ordinary range (`reference` is `ValTy&`/`const ValTy&`), this is equivalent to the DR's `static_cast<T>(*it)` and matches. For a genuine proxy-reference range (`reference` is a proxy type convertible to `ValTy`, not `ValTy&` itself), `reflect_constant(V)` would deduce `T` as the *proxy's* type rather than `ValTy` — likely failing `is_structural_type(^^T)` (proxies are rarely structural) rather than silently misbehaving, so probably fails safely rather than producing a wrong reflection, but not proven either way. **Not yet fixed**: would need `Args.push_back(reflect_constant(static_cast<ValTy>(V)));` to match the DR precisely and handle the proxy case correctly instead of just failing the constraint. Low-risk, mechanical, bundle with the CWG 3111 fix session (same file, same function neighborhood, same build slot). |
