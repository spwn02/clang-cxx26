...
| P3795R2 (Croydon cleanup) | **Partial — scope identification implemented 2026-09-09** | Added `current_function`, `current_class`, and `current_namespace` with compiler-side stack-frame scope lookup, library declarations, value-dependence marking, and `meta::exception` contract checks. Focused test `p3795-scope.pass.cpp` passes. Remaining: tuple metafunctions, `data_member_options.annotations`/generated-member threading, and independent confirmation that parameter annotations are complete. |
