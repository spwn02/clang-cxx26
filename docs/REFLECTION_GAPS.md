...
| P3795R2 (Croydon cleanup) | **Partial — pieces 1, 2, and 4 complete 2026-09-09** | Added the three scope-identification metafunctions and the tuple-related `is_applicable_type`, `is_nothrow_applicable_type`, and `apply_result` library metafunctions. Existing parameter-annotation support was independently verified by `p3394-parameter-annotations.pass.cpp`. Focused tests pass. Remaining: `data_member_options.annotations`/generated-member threading and its Returns/Throws validation. |
