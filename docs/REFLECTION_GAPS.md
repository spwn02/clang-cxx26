...
| NEW-8 | Parameter-only reflection queries accept non-parameter reflections (3 of 4 named functions) | **Reclassified 2026-09-10: no compiler fix justified** | P3096R12 specifies `has_identifier` for entities generally (including class types) and `identifier_of`/`u8identifier_of` for any reflection for which `has_identifier(r)` is true; only the parameter-specific cases add behavior. The existing type-reflection results are conforming. `type_of(^^S)` already rejects. The reverify report’s parameter-only claim was based on an incorrect restriction. | High |
