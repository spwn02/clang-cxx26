...
| 220 | `display_string_of(type_of(undeduced))` hangs | **Confirmed-Open — deferred 2026-09-10, M4 audit** | Exact upstream reproducer was run with `timeout 20` and still exceeded the timeout. Positive control with a body-deduced `auto operator()()` was also prepared, but no fix was attempted. The source path is concrete: `type_of` accepts a `FunctionDecl` whose `FD->getType()` contains an undeduced `AutoType`; the printer then selects the function-type renderer, which recursively queries `return_type_of`, yielding the same undeduced placeholder type and re-entering type rendering. P2996R13's `has-type` condition excludes functions whose type contains an undeduced placeholder, so `type_of` should reject before constructing that reflection. A narrow guard appears conceptually possible, but this shared AST/evaluator path was not patched because available memory was below the documented safe rebuild threshold and no fix could be empirically rebuilt and full-gated in this session. | High |
