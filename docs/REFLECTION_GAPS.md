...
| P3560R2 "Error Handling in Reflection" | **Partial — `meta::exception` and fifteen library-side checks implemented 2026-09-09; compiler-side and remaining Throws rewiring open.** | `libcxx/include/meta` now defines the synopsis shape, consteval constructors/accessors, defaulted special members, and `std::exception` base; `exception.pass.cpp` covers both constructors, all accessors, direct consteval try/catch, and positive layout/member/template-query coverage. Strategy (1) wraps `has_inaccessible_nonstatic_data_members`, `has_inaccessible_bases`, `annotations_of_with_type`, `size_of`, `bit_size_of`, `alignment_of`, `bases_of`, `static_data_members_of`, `nonstatic_data_members_of`, `template_of`, `template_arguments_of`, `access_context::via`, `enumerators_of`, `parameters_of`, and `return_type_of` before compiler dispatch. The remaining Throws-bearing functions still use `DiagFn`; strategy (2) remains needed for failures not checkable from exposed predicates. Nested consteval propagation was re-tested on 2026-09-09 through two user calls and a `std::meta::exception` wrapper; it passes after refreshing staged libc++ headers. See `docs/reflection-audit/codex-nested-throw-report.md`. |
...
**2026-09-09 — P3560R2 strategy (1), batch 3.** Added library-side `meta::exception` precondition
throws to `bases_of`, `static_data_members_of`, and `nonstatic_data_members_of`,
using existing class/namespace reflection predicates. Added positive member-query coverage to
`exception.pass.cpp`; the focused libc++ lit test passed. `members_of` was deliberately excluded
after the full reflection sweep showed that its existing entity-proxy behavior cannot be guarded
by the plain class/namespace predicates without rejecting valid proxy queries.

**2026-09-09 — P3560R2 strategy (1), batch 3 continued.** Added library-side `meta::exception`
precondition throws to `parameters_of` and `return_type_of`, using existing function/function-type
predicates; added positive parameter-query coverage to `exception.pass.cpp`. The focused libc++ lit
test passed.
