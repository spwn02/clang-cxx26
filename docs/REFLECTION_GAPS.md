...
| P3795R2 (Croydon cleanup) | **Partial — scope identification and tuple metafunctions implemented 2026-09-09** | Added the three scope-identification metafunctions and the tuple-related `is_applicable_type`, `is_nothrow_applicable_type`, and `apply_result` library metafunctions. Focused tests `p3795-scope.pass.cpp` and `p3795-tuple.pass.cpp` pass. Remaining: `data_member_options.annotations`/generated-member threading, and independent confirmation that parameter annotations are complete. |
