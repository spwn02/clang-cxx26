...
| NEW-2 | Annotation on empty-declaration is accepted | **Re-verified CONFIRMED 2026-09-10** (`codex-m5-reverify-report.md`) | P3394R4 prohibits an annotation in an empty-declaration's attribute-specifier-seq. Independent repro with `-fannotation-attributes` (the flag that resolved this fork's earlier "which error fires" ambiguity) confirms `[[=1]];` at namespace scope is accepted with no diagnostic. Needs-New-Test → genuine compiler fix + `.verify.cpp` once fixed. | High |
| NEW-3 | Invalid `annotations_of_with_type` arguments are accepted | **Re-verified CONFIRMED 2026-09-10** (`codex-m5-reverify-report.md`) | P3394R4 requires the item and filter arguments to satisfy the specified reflection domains. Independent minimal repro confirms an invalid item/type argument pair is accepted and silently produces an empty result instead of a diagnostic. Needs-New-Test → genuine compiler fix + `.verify.cpp` once fixed. | High |
| NEW-4 | `has_inaccessible_nonstatic_data_members` accepts closure types | **Re-verified CONFIRMED 2026-09-10** (`codex-m5-reverify-report.md`) | P2996R13 requires this query to be ill-formed for a closure type. Independent repro `^^decltype([]{})` passed to `has_inaccessible_nonstatic_data_members` confirms it is accepted with no diagnostic. Needs-New-Test → genuine compiler fix + `.verify.cpp` once fixed. | High |
...
| NEW-6 | Constructor reflection splice is accepted (destructor already correctly rejected) | **Re-verified, narrowed 2026-09-10** (`codex-m5-reverify-report.md`) | Original combined claim overgeneralized. Independent repro shows: `[:^^S::S:]` (constructor splice) **is accepted with no diagnostic — CONFIRMED real gap.** `[:^^S::~S:]` (destructor splice) **is already correctly rejected — this half of the original claim is REFUTED.** Only the constructor-splice case needs a fix + `.verify.cpp` (negative test); the destructor case just needs a positive-control `.verify.cpp` confirming existing correct rejection. | High |
| NEW-7 | Dependent splice-specifier is accepted in forbidden CTAD-like position | **Re-verified CONFIRMED 2026-09-10** (`codex-m5-reverify-report.md`) | P2996R13 requires the dependent splice-specifier form to be rejected in this position. Independent repro reconstructing the exact scenario (dependent `[:R:] value = {1}` for a type with a converting constructor) confirms it instantiates successfully with no diagnostic. Needs-New-Test → genuine compiler fix + `.verify.cpp` once fixed. | High |
| NEW-8 | Parameter-only reflection queries accept non-parameter reflections (3 of 4 named functions) | **Re-verified, narrowed 2026-09-10** (`codex-m5-reverify-report.md`) | Original claim's `type_of` mention was wrong: `type_of(^^S)` already correctly diagnoses (a type reflection has no type) — **REFUTED for `type_of`.** `identifier_of`, `u8identifier_of`, and `has_identifier` called with a non-parameter (type) reflection **are confirmed accepted with no diagnostic — CONFIRMED real gap** for these 3 functions only. Needs-New-Test → genuine compiler fix + `.verify.cpp` for the 3 confirmed functions; `type_of` needs only a positive-control test confirming existing correct rejection. | High |
