...
pre-existing ASTUnit/libclang PCH-loading bug. This 23-count is specifically the
`check-clang` baseline. Separately, **7 pre-existing failures exist in
`libcxx/test/std/experimental/reflection/` under the libc++ lit wrapper**
...
all four fail on `-Werror,-Wdeprecated-declarations` for the header's own deprecated `dealias`
alias to `underlying_entity_of`; `m5-p2996-batch13.verify.cpp` fails on unused-variable warnings
for probe-only locals; `m5-p2996-p3096-batch12.verify.cpp` and `m5-p3491-batch15.verify.cpp`
have similar pre-existing warning/verify mismatches). **Confirmed via stash-isolation on
2026-09-10 (same 7 fail on an unmodified checkout) — unrelated to any reflection-epic fix,
do not treat as a regression.** Do not count this cluster against a new fix's gate either. Do not
count either cluster as a
...
3. **Consteval self-reference escalation cluster — investigated 2026-09-08, deliberately NOT
   ...
   **Re-attempted 2026-09-10 (dedicated session, two independent strategies, both rejected —
   see [`codex-m3-escalation-report.md`](reflection-audit/codex-m3-escalation-report.md)).**
   `docs/LLVM22_SYNC.md` no longer exists (deleted by a different epic's close-out); the regression
   set was re-derived empirically instead of reused, and **has grown from 9 to 25 libc++ tests**
   plus one new Clang-side regression (`Reflection/reflection-wording-examples.cpp`) — this epic's
   own M5 test-writing work since August added most of the newly-affected files, so the historical
   9-test number is now stale and should not be reused as the regression gate.
   - **Strategy A (reapply the original reverted fix verbatim).** Confirmed the exact prior
     result: fixes both self-reference cases
     (`SemaCXX/builtin-is-within-lifetime.cpp`, `SemaCXX/constant-expression-cxx11.cpp`) but
     produces the 25-test libc++ regression plus the new Clang one.
   - **Confirmed the init-attachment-timing hypothesis from this session's earlier analysis**,
     empirically, not just by static reading: `ParseDecl.cpp:2551-2563`'s initializer RAII scope
     calls `ActOnCXXExitDeclInitializer` (hence `HandleImmediateInvocations`) *before*
     `ParseDecl.cpp:2704`'s `AddInitializerToDecl` ever calls `VDecl->setInit(...)`
     (`SemaDecl.cpp:13842`). So `VarDecl::isUsableInConstantExpressions()`/`hasConstantInitialization()`
     — the exact signal `HandleImmediateInvocations`' "manifestly constant-evaluated" bailout
     (`SemaExpr.cpp:18404-18421`) checks — are unconditionally false at this call site, for every
     declaration, fork or upstream. This *is* real and *is* a contributing cause, exactly as
     hypothesized.
   - **Strategy B (per-candidate success/failure signal, syntactic constexpr/constinit bailout
     signal instead of the dead attached-init one)** — the fix design this session's own analysis
     and the 2026-09-08 entry above both converged on. Implemented around
     `SemaExpr.cpp:18144-18224,18400-18560`: track which `ImmediateInvocationCandidate`s evaluate
     successfully, skip `err_expr_consteval_only_type` for those, and gate the manifestly-constant-
     evaluated bailout on `VD->isConstexpr()`/`ConstInitAttr` (available pre-attachment) instead of
     the dead `isUsableInConstantExpressions()` check — but *only* when no candidate in the record
     failed (so self-reference still surfaces). **Built successfully but still rejected by focused
     lit**: produced spurious `err_expr_consteval_only_type` on ordinary non-consteval reflection
     declarations, altered expected-warning behavior in the C++23 lifetime test, and regressed
     `Reflection/consteval-only-types.cpp` (the smuggling test itself). Confirms the 2026-09-08
     entry's own conclusion was already correct and now has code-level, not just architectural,
     evidence: **a per-candidate bit is not sufficient in isolation** — the shared context flag
     conflates at least three distinct diagnostic concerns (ordinary immediate-function-context
     escalation, consteval-only-type smuggling suppression, and self-reference-in-initializer
     escalation), and relaxing it for "any successfully-evaluated candidate" leaks into the other
     two. All temporary edits reverted; tree restored to baseline (verified clean, no diff).
   - **Status: confirmed genuinely hard, not a quick-fix candidate, across three independent
     attempts now** (the original August fix, this epic's static-analysis pass, and this session's
     two built-and-tested strategies) **all converging on the same root cause and the same
     conclusion** — a correct fix needs the compiler to track manifestly-constant-evaluation and
     candidate-success state *per initializer context*, not as one shared boolean context flag, which
     is a real (if scoped) `ExpressionEvaluationContextRecord` architecture change, not a
     patch-sized fix. This is the one item where the epic's own completion criterion #4 ("fixed and
     verified") is not yet met and, per the evidence above, will not be met without a session
...
| NEW-4 | `has_inaccessible_nonstatic_data_members` accepts closure types | **Fixed-and-verified 2026-09-10** — this commit | Added compiler closure-type detection and the required `meta::exception` path; [new-4-closure-inaccessible.verify.cpp](../libcxx/test/std/experimental/reflection/new-4-closure-inaccessible.verify.cpp) covers rejection and a valid class. | High |
...
| NEW-6 | Constructor reflection splice is accepted (destructor already correctly rejected) | **Fixed-and-verified 2026-09-10** — this working-tree change | Constructor reflections now use the existing invalid splice-type diagnostic; [new-6-constructor-splice.verify.cpp](../libcxx/test/std/experimental/reflection/new-6-constructor-splice.verify.cpp) covers rejection and a valid type splice. Destructor handling is unchanged. | High |
| NEW-7 | Dependent splice-specifier is accepted in forbidden CTAD-like position | **Deferred-with-reason 2026-09-10, revised** (`codex-new7-design-report.md` + personal follow-up) | A Codex design session proposed a narrow `AddInitializerToDecl` rejection for copy-list-initialized dependent splice types, exempting an explicit `typename`. Personal verification caught **three distinct correctness bugs** across two implementation attempts before this was reverted (no source/test change is in the tree): (1) the session's own exemption check read `getTypenameKWLoc()` off the `ReflectionSpliceType`, but this fork's `TST_type_splice` path doesn't set that reliably, so it fell back to scanning the *raw source line* for the substring `"typename"` — a comment containing that word anywhere earlier on the declaration's line silently suppressed the diagnostic (confirmed with a direct repro: `/* typename */ [:R:] value = {1};` compiled clean when it must not). (2) Replacing that with a pure AST-based check (`getTypenameKWLoc().isInvalid()`, no text scan) broke a *real, load-bearing* existing test — `clang/test/Reflection/splice-templates.cpp`'s `DepTClsCTAD` (`typename [:R:] obj = {value};`, a dependent, typename-qualified, copy-list-initialized splice) started failing, proving the exemption is normatively necessary here, not just cosmetic. (3) A third attempt used `Lexer::findPreviousToken` to precisely find the actual token immediately before the splice's `[:` (avoiding both the raw-line-scan false negative and the AST-based false positive) — verified correct in 3 isolated single-function repros (comment-decoy rejected, genuine `typename[:R:]` accepted, plain dependent copy-list-init rejected) — but then failed unpredictably (wrong diagnostics/missing warnings across unrelated declarations) once multiple such templates coexisted in one translation unit, matching this file's actual shape (`new-7-dependent-splice.verify.cpp`, 6 template functions). Root cause of that multi-declaration interaction not identified before time-boxing this attempt; likely a pattern-vs-instantiation double-checking or location-reuse issue specific to how this fork represents dependent splice types across sibling templates. **No fix is in the tree.** A future attempt needs to understand exactly when `AddInitializerToDecl` runs for a *dependent* splice-typed declarator (pattern-definition time vs. per-instantiation, and whether both happen) before trying another location- or AST-based discriminator. | High |
| ~~NEW-8~~ | ~~Parameter-only reflection queries accept non-parameter reflections~~ | **Closed 2026-09-10: not a gap, checklist row corrected** | P3096R12 specifies `has_identifier` for entities generally (including class types) and `identifier_of`/`u8identifier_of` for any reflection for which `has_identifier(r)` is true; only the parameter-specific cases add behavior. The existing type-reflection results are conforming — no compiler fix needed. `type_of(^^S)` already rejects and is unchanged/out of scope. M5 checklist row 3096-06's condition text was corrected to match, and marked Covered via [identifier-of-non-parameter.pass.cpp](../libcxx/test/std/experimental/reflection/identifier-of-non-parameter.pass.cpp) (positive control for all 3 functions on type/member/function/parameter reflections). | High |
...
| 220 | `display_string_of(type_of(undeduced))` hangs | **Fixed-and-verified 2026-09-10** | `type_of`'s `ReflectionKind::Declaration` case (`ExprConstantMeta.cpp`, in the `FunctionDecl` branch) now rejects via the existing `diag::metafn_undeduced_placeholder` note (already used by `substitute()`'s function-template-specialization path for the identical condition) when `FD->getType()->isUndeducedType()`, matching P2996R13's has-type exclusion for a function whose type contains an undeduced placeholder. Exact upstream reproducer (`timeout 20 ...`) now completes instantly with a clean diagnostic instead of hanging. [issue-220-undeduced-type-of.verify.cpp](../libcxx/test/std/experimental/reflection/issue-220-undeduced-type-of.verify.cpp) covers the negative case plus positive controls (a deduced-return-type member function and an ordinary non-auto function). Verified: clang-side `Reflection` suite 20/20, libcxx `reflection` suite matches the known 7-failure baseline (0 new), `SemaCXX` suite matches the known 5-failure consteval-escalation baseline (0 new). | High |
...
**2026-09-10 — M4 issues #188/#220/#237 empirical audit.** Read the exact tracker rows and
local upstream issue snapshot reproducers. #220 still hangs past `timeout 20`; source tracing
identifies `type_of` accepting an undeduced `AutoType` followed by recursive function-type
printing through `return_type_of`. #237 still rejects the exact closure alias; current adopted
splice wording requires a type-designating splice and provides a type-only alias example, so it
is a real deeper alias/type gap rather than Not-Applicable. #188's exact `ranges::max_element`
sequence reproduces five failures (`1.2`, `2.2`, `2.3`, `3.3`, `4.3`) at the
`pretty_printer::print`/`reflect_invoke` constant-evaluation boundary. No source fixes were
attempted: none was narrow and safely verifiable under the current memory pressure. Temporary
probes were removed. Full post-fix gates were not applicable because no fix was made.
...
(complete annotation filter types), with positive controls in four new `.verify.cpp` tests. NEW-7
was deferred after tracing dependent splice-type initialization because no narrow safe rejection
hook was found. NEW-8 was reclassified after reading P3096R12: the paper does not make the three
queries parameter-only, so the accepted non-parameter probes are conforming. Full Clang lit
completed through `libcxx-lit -j1` with escalation after sandbox forkserver denial at the
23-failure baseline. The libc++ wrapper discovered 11,832 tests, skipped 11,826, and ran six
passing tests before interruption. Git commit/push is blocked by the repository `.git` read-only
filesystem.
...
**2026-09-10 — Reflection Closure NEW-7 design investigation.** Read adopted P2996R13 CTAD
wording, traced ordinary CTAD and dependent splice construction, and implemented a narrow
copy-list-initialization diagnostic in `AddInitializerToDecl`. Added the focused NEW-7 verify test
and updated batch 13. Built Clang with `env CCACHE_DISABLE=1 ninja -C build-nyx -j1 clang`;
...
### 2026-09-10 (later) — Re-audit against the plan's own 7-point completion definition; M3 escalation cluster reopened, CWG 3111/LWG 4432 audited

M6-complete does not mean epic-complete. Consulted the advisor after committing M6 and
re-reading the plan's "Definition of genuinely complete" (7 criteria) against current tracker
state: criterion #4 requires the escalation cluster **fixed and verified**, not accepted as
baseline — but M6's own clean check-clang verdict is conditional on treating that cluster's 5
SemaCXX failures as acceptable baseline. Criteria #3 (CWG 3111/LWG 4432) were still "Not yet
checked". These are real gaps against the epic's own bar, not close-enough. Criterion #7's
"zero open rows" is read against criterion #1's own "documented reason" allowance — a row with a
documented, reasoned disposition (including "confirmed real, deferred, here's why") satisfies it;
a row with no disposition at all would not. Under that reading most of the tracker's
`Confirmed-Open`/`Needs-Build-To-Verify` rows already qualify; this was not separately re-audited
row-by-row this session (flagged as remaining work below).

**M3 escalation cluster (criterion #4) reopened.** Dispatched a fresh Codex session
(`docs/reflection-audit/codex-m3-escalation-report.md`, in flight — not yet returned at the time
of this entry) with: (1) the exact regression-set-first discipline the advisor specified — reapply
the old reverted fix, rebuild, and capture the *current* exact regressed-test list before designing
anything, since `docs/LLVM22_SYNC.md`'s original list no longer exists in this tree (deleted by a
different epic's close-out) and the epic's own current test surface has moved since August; (2) a
new, previously-untested hypothesis found by re-reading `HandleImmediateInvocations`
(`SemaExpr.cpp:18396-18560`) and `VarDecl::isUsableInConstantExpressions`
(`clang/lib/AST/Decl.cpp:2552`): the "manifestly constant-evaluated" early-bailout in
`HandleImmediateInvocations` (`SemaExpr.cpp:18404-18421`) checks
`isUsableInConstantExpressions()`/`hasConstantInitialization()`, both of which require the
VarDecl's initializer to already be attached (`getAnyInitializer`) — but this function runs from
`ActOnCXXExitDeclInitializer`, which fires before `AddInitializerToDecl` attaches the initializer.
If confirmed, that bailout is dead code at this call site regardless of fork/upstream, which would
explain why removing the `ImmediateFunctionContext` push (the previously-reverted fix) produced
spurious `err_expr_consteval_only_type` on the ordinary `constexpr auto R = <consteval-fn>();`
idiom: nothing catches it. The fix, if this holds, is to gate the bailout on the same syntactic
signal `ActOnCXXEnterDeclInitializer` already uses (`VD->isConstexpr()` / constinit), not on
initializer-dependent queries — but per the 2026-09-08 entry's own finding, that alone would
re-suppress the genuine self-reference diagnostic, so the real fix likely also needs a
per-candidate success/failure signal (not just a context-wide flag) threaded from
`CheckForImmediateInvocation`'s already-evaluated-successfully path into the `Rec.ConstevalOnly`
diagnosis loop. Told Codex to verify empirically (temporary tracing if needed), not assume, and to
iterate rather than one-shot; explicitly authorized to take as long as needed and to stop and
document rather than ship a partial fix, matching the NEW-7 discipline. Will personally verify any
claimed fix with direct repros before trusting it, per this epic's established validator discipline
— a Codex "fixed and verified" claim is not sufficient on its own.

**CWG 3111 / LWG 4432 (criterion #3) audited, both now have concrete findings** (see the DR table
above) instead of "Not yet checked". CWG 3111: confirmed real gap, `reflect_constant`'s array-type
path has no `TemplateParamObjectDecl`-equivalent handling, compounded by an independent
synopsis/definition signature mismatch (`const T&` declared vs. `T` by-value defined) that makes
the array case unreachable in the first place. LWG 4432: likely fine for ordinary ranges, one
unverified proxy-reference edge case, low-risk mechanical fix identified
(`static_cast<ValTy>(V)` before `reflect_constant`). Both fixes scoped but **not yet built or
tested** — queued for the same build slot as the M3 escalation-cluster fix, after the Codex session
above returns, to respect the plan's single-coordinator build-serialization rule (a second
concurrent `ninja`/`lit` run against the same tree risks corrupting both).

**Not yet done, explicitly deferred to the next turn(s) of this session:** re-verify the M3 Codex
session's findings personally, build+test the CWG 3111/LWG 4432 fixes, and a full row-by-row
criterion #7 re-audit of the ~35 `Confirmed-Open`/12 `Needs-Build-To-Verify` rows to confirm each
genuinely carries a documented disposition (not just a "not yet checked" placeholder). M7 close-out
remains not-started and should not start until this list is worked down further.

### 2026-09-10 (later still) — CWG 3111 / LWG 4432 fixed and verified

Two Codex build/verify rounds after the hand-written design (see the DR table above for
the full technical writeup). Round 1 found the initial single-overload `const T&` design
silently dropped `reflect_constant`'s copy-constructibility Mandate and couldn't cleanly
handle nested arrays; redesigned as two overloads (a new array-specific one, the original
completely untouched) to avoid both problems structurally rather than patching around
them. Round 2 confirmed the redesign: new test passes, the previously-regressed
`m5-p2996-batch1.verify.cpp` NonCopyable diagnostic is restored, full reflection subtree
at the documented 7-failure baseline (net +1 passing test, 0 new failures), no
compiler-side change needed. Cleaned up one stale comment referencing an earlier design's
symbol (`remove_extent_t<T>`, no longer used) before committing — comment-only, re-checked
the diff contained no executable-line changes so did not require a third build round.
Committed and pushed. Both DR rows in the paper-audit table are now Fixed-and-verified.

Criterion #3 (CWG 3111, LWG 4432, LWG 4426, LWG 4428) is now fully resolved: LWG 4426 and
4428 were already dispositioned in earlier sessions (see their table rows above), and the
two remaining ones are closed here. Criterion #4 (three known bugs) remains the one
outstanding item — the escalation cluster is confirmed genuinely hard (three independent
attempts, see the M3 section above), not fixed. Next: either take on the escalation
cluster's architecture-level fix (a session specifically budgeted for restructuring
`ExpressionEvaluationContextRecord`'s per-candidate state, not a quick patch), or do the
criterion #7 row-by-row audit of `Confirmed-Open`/`Needs-Build-To-Verify` rows first since
it's lower-risk and independently valuable. Not yet decided; next session's judgment call.
