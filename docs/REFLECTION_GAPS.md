...
| NEW-7 | Dependent splice-specifier is accepted in forbidden CTAD-like position | **Deferred-with-reason 2026-09-10** (`codex-m5-reverify-report.md`) | Traced dependent `[:R:]` through `BuildReflectionSpliceType` into later declaration initializer/deduction handling. No narrow rejection hook was identified that distinguishes this forbidden CTAD-like case from valid dependent direct-list initialization; broader changes risk regressions. No implementation or status flip. | High |
