...
#    endif
  ...
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI bool operator()(
      _Ep&& __exec, _Iter __first, _Sent __last, const _Type& __value, _Proj __proj = {}) const {
    _Iter __end = __first + (__last - __first);
    return ranges::find(std::forward<_Ep>(__exec), std::move(__first), std::move(__end), __value, std::move(__proj)) != __end;
  }
...
#    endif
  ...
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI bool operator()(
      _Ep&& __exec, _Range&& __range, const _Type& __value, _Proj __proj = {}) const {
