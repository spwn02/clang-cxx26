...
#  if _LIBCPP_HAS_EXPERIMENTAL_PSTL
  template <class _Ep, random_access_iterator _Iter1, sized_sentinel_for<_Iter1> _Sent1,
    ...
    requires indirectly_comparable<_Iter1, _Iter2, _Pred, _Proj1, _Proj2>
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI _Iter1 operator()(
    ...
    return std::find_first_of(std::forward<_Ep>(__exec), std::move(__first1), std::move(__end1),
                              std::move(__first2), std::move(__end2),
  ...
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI borrowed_iterator_t<_Range1> operator()(
