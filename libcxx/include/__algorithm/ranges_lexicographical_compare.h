...
#  if _LIBCPP_HAS_EXPERIMENTAL_PSTL
  template <class _Ep,
            ...
            sized_sentinel_for<_Iter1> _Sent1,
            random_access_iterator _Iter2,
            sized_sentinel_for<_Iter2> _Sent2,
            class _Proj1                                                                           = identity,
            class _Proj2                                                                           = identity,
            indirect_strict_weak_order<projected<_Iter1, _Proj1>, projected<_Iter2, _Proj2>> _Comp = ranges::less,
  ...
  _LIBCPP_HIDE_FROM_ABI bool operator()(
    ...
    return std::lexicographical_compare(
        ...
        std::move(__first2),
        __end2,
        [__comp = std::move(__comp), __proj1 = std::move(__proj1), __proj2 = std::move(__proj2)](
            auto&& __a, auto&& __b) mutable {
          return std::invoke(__comp,
                             std::invoke(__proj1, std::forward<decltype(__a)>(__a)),
  ...
  _LIBCPP_HIDE_FROM_ABI bool operator()(
      _Ep&& __exec, _Range1&& __range1, _Range2&& __range2, _Comp __comp = {}, _Proj1 __proj1 = {}, _Proj2 __proj2 = {})
      const {
    return (*this)(
        std::forward<_Ep>(__exec),
        ranges::begin(__range1),
