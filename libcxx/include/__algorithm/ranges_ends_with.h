...
#  if _LIBCPP_HAS_EXPERIMENTAL_PSTL
  ...
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI bool operator()(
    ...
    if (__len2 > __len1)
      return false;
    return ranges::equal(std::forward<_Ep>(__exec), __first1 + (__len1 - __len2), __first1 + __len1,
                         __first2, __first2 + __len2, std::move(__pred), std::move(__proj1), std::move(__proj2));
  }
