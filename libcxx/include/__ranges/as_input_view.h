...
public:
  ...
  constexpr explicit as_input_view(_View __base) : __base_(std::move(__base)) {}

  constexpr _View base() const& requires copy_constructible<_View> { return __base_; }
  constexpr _View base() && { return std::move(__base_); }
  constexpr auto begin() requires(!__simple_view<_View>) { return __iterator<false>(ranges::begin(__base_)); }
  constexpr auto begin() const requires range<const _View> { return __iterator<true>(ranges::begin(__base_)); }
  constexpr auto end() requires(!__simple_view<_View>) { return ranges::end(__base_); }
  constexpr auto end() const requires range<const _View> { return ranges::end(__base_); }
...
public:
  ...
  constexpr __iterator(__iterator<!_Const> __other)
  ...
  constexpr __iterator& operator++() { ++__current_; return *this; }
  constexpr void operator++(int) { ++*this; }
  friend constexpr bool operator==(const __iterator& __it, const sentinel_t<_Base>& __sent) {
    return __it.__current_ == __sent;
  }
  friend constexpr range_difference_t<_Base> operator-(const sentinel_t<_Base>& __sent, const __iterator& __it)
      requires sized_sentinel_for<sentinel_t<_Base>, iterator_t<_Base>> { return __sent - __it.__current_; }
  friend constexpr range_difference_t<_Base> operator-(const __iterator& __it, const sentinel_t<_Base>& __sent)
      requires sized_sentinel_for<sentinel_t<_Base>, iterator_t<_Base>> { return __it.__current_ - __sent; }
  friend constexpr range_rvalue_reference_t<_Base> iter_move(const __iterator& __it)
...
inline constexpr bool enable_borrowed_range<as_input_view<_View>> = enable_borrowed_range<_View>;
