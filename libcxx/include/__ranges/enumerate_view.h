...
concept __range_with_movable_references =
    input_range<_Range> && move_constructible<range_reference_t<_Range>> &&
...
public:
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr auto begin() const requires __range_with_movable_references<const _View> {
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr auto end() const requires __range_with_movable_references<const _View> {
...
class enumerate_view<_View>::__iterator {
  ...
  static consteval auto __get_iterator_concept() {
    ...
    else if constexpr (forward_range<_Base>) return forward_iterator_tag{};
...
public:
  _LIBCPP_HIDE_FROM_ABI __iterator() requires(default_initializable<iterator_t<_Base>>) = default;
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr __iterator& operator++() { ++__current_; ++__pos_; return *this; }
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr __iterator& operator+=(difference_type __n) requires random_access_range<_Base> {
    __current_ += __n; __pos_ += __n; return *this;
  }
  _LIBCPP_HIDE_FROM_ABI constexpr __iterator& operator-=(difference_type __n) requires random_access_range<_Base> {
    __current_ -= __n; __pos_ -= __n; return *this;
  }
  _LIBCPP_HIDE_FROM_ABI constexpr auto operator[](difference_type __n) const requires random_access_range<_Base> {
  ...
  _LIBCPP_HIDE_FROM_ABI friend constexpr strong_ordering operator<=>(const __iterator& __x, const __iterator& __y) noexcept {
    return __x.__pos_ <=> __y.__pos_;
  }
  _LIBCPP_HIDE_FROM_ABI friend constexpr __iterator operator+(const __iterator& __i, difference_type __n)
    requires random_access_range<_Base> { auto __temp = __i; __temp += __n; return __temp; }
  _LIBCPP_HIDE_FROM_ABI friend constexpr __iterator operator+(difference_type __n, const __iterator& __i)
    requires random_access_range<_Base> { return __i + __n; }
  _LIBCPP_HIDE_FROM_ABI friend constexpr __iterator operator-(const __iterator& __i, difference_type __n)
    requires random_access_range<_Base> { auto __temp = __i; __temp -= __n; return __temp; }
  _LIBCPP_HIDE_FROM_ABI friend constexpr difference_type operator-(const __iterator& __x, const __iterator& __y) noexcept {
    return __x.__pos_ - __y.__pos_;
  }
...
public:
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr __sentinel(__sentinel<!_Const> __other)
    requires _Const && convertible_to<sentinel_t<_View>, sentinel_t<_Base>> : __end_(std::move(__other.__end_)) {}
  ...
  operator-(const __iterator<_OtherConst>& __x, const __sentinel& __y) { return __get_current(__x) - __y.__end_; }
  template <bool _OtherConst>
    requires sized_sentinel_for<sentinel_t<_Base>, iterator_t<__maybe_const<_OtherConst, _View>>>
  _LIBCPP_HIDE_FROM_ABI friend constexpr range_difference_t<__maybe_const<_OtherConst, _View>>
  operator-(const __sentinel& __x, const __iterator<_OtherConst>& __y) { return __x.__end_ - __get_current(__y); }
};

template <class _View>
inline constexpr bool enable_borrowed_range<enumerate_view<_View>> = enable_borrowed_range<_View>;
