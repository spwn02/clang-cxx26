...
// [optional.syn]: format_kind<optional<T>> == range_format::disabled is defined in <optional>
// itself (C++26), not here -- it only needs the lightweight <__format/range_format.h> that
// <optional> already includes, not this header's much heavier formatter machinery. Defining it
// here too, even behind a local forward declaration of optional to avoid #include <optional>,
// would be a duplicate/ODR-conflicting definition in any translation unit that includes both.
