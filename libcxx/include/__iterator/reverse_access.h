...
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 reverse_iterator<_Tp*> rbegin(_Tp (&__array)[_Np]) _NOEXCEPT {
  return reverse_iterator<_Tp*>(__array + _Np);
}

template <class _Tp, size_t _Np>
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 reverse_iterator<_Tp*> rend(_Tp (&__array)[_Np]) _NOEXCEPT {
  return reverse_iterator<_Tp*>(__array);
}

template <class _Ep>
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 reverse_iterator<const _Ep*>
rbegin(initializer_list<_Ep> __il) _NOEXCEPT {
  return reverse_iterator<const _Ep*>(__il.end());
}

template <class _Ep>
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 reverse_iterator<const _Ep*>
rend(initializer_list<_Ep> __il) _NOEXCEPT {
...
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 auto crbegin(const _Cp& __c)
    noexcept(noexcept(std::rbegin(__c))) -> decltype(std::rbegin(__c)) {
  return std::rbegin(__c);
}

template <class _Cp>
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 auto crend(const _Cp& __c)
    noexcept(noexcept(std::rend(__c))) -> decltype(std::rend(__c)) {
  return std::rend(__c);
}
