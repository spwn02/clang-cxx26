...
static_assert(CanErase<std::set<std::string, TransparentComparator>>);
static_assert(!CanErase<const std::set<std::string, TransparentComparator>>);
static_assert(!CanErase<std::set<std::string, NonTransparentComparator>>);

int main(int, char**) {
  // Erase via a heterogeneous (non-key_type) argument.
  {
    std::set<std::string, TransparentComparator> c = {"a", "b", "c"};
