...
static_assert(CanErase<std::multiset<std::string, TransparentComparator>>);
static_assert(!CanErase<const std::multiset<std::string, TransparentComparator>>);
static_assert(!CanErase<std::multiset<std::string, NonTransparentComparator>>);

int main(int, char**) {
  // Erase via a heterogeneous (non-key_type) argument.
  {
    std::multiset<std::string, TransparentComparator> c = {"a", "a", "b"};
