...
static_assert(CanErase<std::unordered_multimap<std::string, int, TransparentHash, std::equal_to<>>>);
static_assert(!CanErase<const std::unordered_multimap<std::string, int, TransparentHash, std::equal_to<>>>);
static_assert(!CanErase<std::unordered_multimap<std::string, int, NonTransparentHash>>);

int main(int, char**) {
  // Erase via a heterogeneous (non-key_type) argument.
  {
    std::unordered_multimap<std::string, int, TransparentHash, std::equal_to<>> c = {{"a", 1}, {"a", 2}, {"b", 3}};
