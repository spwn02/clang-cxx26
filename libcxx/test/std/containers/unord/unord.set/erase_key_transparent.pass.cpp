...
static_assert(CanErase<std::unordered_set<std::string, TransparentHash, std::equal_to<>>>);
static_assert(!CanErase<const std::unordered_set<std::string, TransparentHash, std::equal_to<>>>);
static_assert(!CanErase<std::unordered_set<std::string, NonTransparentHash>>);

int main(int, char**) {
  // Erase via a heterogeneous (non-key_type) argument.
  {
    std::unordered_set<std::string, TransparentHash, std::equal_to<>> c = {"a", "b", "c"};
