...
// constexpr ForwardIt uninitialized_copy(InputIt, InputIt, ForwardIt);
// template <class InputIt, class Size, class ForwardIt>
// constexpr ForwardIt uninitialized_copy_n(InputIt, Size, ForwardIt);
// namespace ranges { ... } // constexpr overloads, see [uninitialized.copy]
//
...
constexpr bool test_classic() {
  ...
  return with_allocated<int>(3, [&](int* p, std::size_t n) {
    auto result = std::uninitialized_copy(src, src + n, p);
...
constexpr bool test_classic_n() {
  int src[3] = {1, 2, 3};
  return with_allocated<int>(3, [&](int* p, std::size_t n) {
    auto result = std::uninitialized_copy_n(src, n, p);
    return result == p + n && p[0] == 1;
  });
}
...
constexpr bool test_ranges_iter() {
  int src[3] = {1, 2, 3};
  return with_allocated<int>(3, [&](int* p, std::size_t n) {
    auto result = std::ranges::uninitialized_copy(src, src + n, p, p + n);
...
constexpr bool test_ranges_range() {
  ...
  return with_allocated<int>(3, [&](int* p, std::size_t n) {
    ...
    auto result = std::ranges::uninitialized_copy(in, out);
    return result.out == p + n && p[0] == 1;
  });
}
...
constexpr bool test_ranges_n() {
  int src[3] = {1, 2, 3};
  return with_allocated<int>(3, [&](int* p, std::size_t n) {
    auto result = std::ranges::uninitialized_copy_n(src, n, p, p + n);
    return result.out == p + n && p[0] == 1;
  });
}
